...
OUTDIR=/data1/jingyixi/wm_runs/state_rollout_alignment_fastpilot
mkdir -p "$OUTDIR"
...
train_one() {
  ...
  local tag=$2
  local weight=$3
  local subdir="pusht_encoder_moda_v14_full_visible_bs32_pred6_gate07_staterollfast_${tag}"
  local model="lewm_encoder_moda_v14_full_visible_bs32_pred6_gate07_staterollfast_${tag}"
...
eval_one() {
  ...
  local ep=$3
  local subdir="pusht_encoder_moda_v14_full_visible_bs32_pred6_gate07_staterollfast_${tag}"
  local model="lewm_encoder_moda_v14_full_visible_bs32_pred6_gate07_staterollfast_${tag}_epoch_${ep}"
  local ckpt="$STABLE/$subdir/${model}_object.ckpt"
  if [[ ! -f "$ckpt" ]]; then
    echo "[SKIP_EVAL] missing $ckpt"
    return 0
  fi
  local out="staterollfast_${tag}_ep${ep}_seed42_h4_s300_k30_n30.txt"
...
run_branch() {
  ...
  local tag=$2
  local weight=$3
  train_one "$gpu" "$tag" "$weight"
  for ep in 1 2 4; do eval_one "$gpu" "$tag" "$ep"; done
  for ep in 1 2 4; do probe_one "$gpu" "$tag" "$ep"; done
}

run_branch 2 l005 0.05 > "$OUTDIR/gpu2_l005.master.log" 2>&1 &
run_branch 3 l010 0.10 > "$OUTDIR/gpu3_l010.master.log" 2>&1 &
wait
...
outdir = Path('/data1/jingyixi/wm_runs/state_rollout_alignment_fastpilot')
rows = []
for tag in ['l005', 'l010']:
    subdir = stable / f'pusht_encoder_moda_v14_full_visible_bs32_pred6_gate07_staterollfast_{tag}'
    for ep in [1, 2, 4]:
        eval_file = subdir / f'staterollfast_{tag}_ep{ep}_seed42_h4_s300_k30_n30.txt'
        success = None
        if eval_file.exists():
