...
class StateHead(torch.nn.Module):
...
def lejepa_forward(self, batch, stage, cfg):
    ...
    reach_cfg = cfg.loss.get("reachability_contrast", {})
    reach_enabled = bool(reach_cfg.get("enabled", False))
    reach_loss = pred_emb.new_tensor(0.0)
    reach_acc = pred_emb.new_tensor(0.0)
    reach_pos_cost_mean = pred_emb.new_tensor(0.0)
    reach_neg_cost_mean = pred_emb.new_tensor(0.0)
    if reach_enabled:
        target_idx = min(pred_emb.shape[1] - 1, tgt_emb.shape[1] - 1)
        query = pred_emb[:, target_idx].float()
        key = tgt_emb[:, target_idx].detach().float()
        if bool(reach_cfg.get("normalize", False)):
            query = torch.nn.functional.normalize(query, dim=-1)
            key = torch.nn.functional.normalize(key, dim=-1)
        metric = str(reach_cfg.get("metric", "l2"))
        temperature = float(reach_cfg.get("temperature", 0.1))
        if metric == "cosine":
            logits = query @ key.T / max(temperature, 1e-6)
            pos_cost = 1.0 - torch.diag(logits) * max(temperature, 1e-6)
            neg_cost = 1.0 - logits * max(temperature, 1e-6)
        else:
            dist = torch.cdist(query, key, p=2).pow(2)
            logits = -dist / max(temperature, 1e-6)
            pos_cost = torch.diag(dist)
            mask = ~torch.eye(dist.shape[0], dtype=torch.bool, device=dist.device)
            neg_cost = dist[mask].reshape(dist.shape[0], -1)
        labels = torch.arange(logits.shape[0], device=logits.device)
        reach_loss = torch.nn.functional.cross_entropy(logits, labels)
        reach_acc = (logits.argmax(dim=1) == labels).float().mean()
        reach_pos_cost_mean = pos_cost.mean()
        reach_neg_cost_mean = neg_cost.mean()
        output["reachability_contrast_loss"] = reach_loss
        output["reachability_contrast_acc"] = reach_acc.detach()
        output["reachability_pos_cost"] = reach_pos_cost_mean.detach()
        output["reachability_neg_cost"] = reach_neg_cost_mean.detach()
    ...
    if state_enabled:
        ...
        if "state" not in batch:
            raise RuntimeError("loss.state_rollout.enabled=true requires batch['state']")
    ...
    reach_weight = float(reach_cfg.get("weight", 0.0)) if reach_enabled else 0.0
    ...
    output["loss"] = (
        ...
        + reach_weight * reach_loss
        + state_weight * state_rollout_loss
    )
    ...
    if reach_enabled:
        losses_dict[f"{stage}/reachability_contrast_acc"] = output["reachability_contrast_acc"]
        losses_dict[f"{stage}/reachability_pos_cost"] = output["reachability_pos_cost"]
        losses_dict[f"{stage}/reachability_neg_cost"] = output["reachability_neg_cost"]
    if state_enabled:
        losses_dict[f"{stage}/state_rollout_r2_mean"] = output["state_rollout_r2_mean"]
...
def build_clean_encoder_moda_jepa(cfg):
    ...
    if bool(state_cfg.get("enabled", False)):
        ...
        hidden_dim = int(state_cfg.get("hidden_dim", 256))
...
def apply_freeze_cfg(model, cfg):
    train_cfg = cfg.get("train", {})
    if bool(train_cfg.get("freeze_encoder", False)):
        for param in model.encoder.parameters():
            param.requires_grad_(False)
        if bool(train_cfg.get("freeze_projector", True)):
            for param in model.projector.parameters():
                param.requires_grad_(False)
        print("[Freeze] encoder frozen; projector_frozen=" f"{bool(train_cfg.get('freeze_projector', True))}")
    return model
...
def run(cfg):
    ...
    world_model = apply_freeze_cfg(world_model, cfg)
