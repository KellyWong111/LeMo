"""Training entrypoint for the historical exact-kernel MoDA path."""
...
from moda_module_exact import MODA_TRITON_AVAILABLE, MoDAARPredictorExact
...
def ensure_exact_cfg(cfg):
    if not MODA_TRITON_AVAILABLE:
        raise RuntimeError(
            "train_moda_exact.py requires fla.ops.moda.parallel_moda."
        )
    with open_dict(cfg):
        cfg.use_moda = True
        if "depth_start_layer" not in cfg:
            cfg.depth_start_layer = 1
        if "cache_window" not in cfg:
            cfg.cache_window = 0
        if "detach_cache" not in cfg:
            cfg.detach_cache = False
        if "chunk_visible" not in cfg:
            cfg.chunk_visible = False
    return cfg
...
def run(cfg):
    ...
    predictor = MoDAARPredictorExact(
        ...
        cache_window=int(cfg.cache_window),
        detach_cache=bool(cfg.detach_cache),
        chunk_visible=bool(cfg.chunk_visible),
        **cfg.predictor,
    )
    cache_mode = (
        "disabled"
        if int(cfg.depth_start_layer) >= int(cfg.predictor.depth)
        else f"enabled_from_layer_{int(cfg.depth_start_layer)}"
    )
    print(
        "[MoDAExact] "
        f"use_moda={bool(cfg.use_moda)} "
        f"predictor_depth={int(cfg.predictor.depth)} "
        f"depth_start_layer={int(cfg.depth_start_layer)} "
        f"cache_window={int(cfg.cache_window)} "
        f"detach_cache={bool(cfg.detach_cache)} "
        f"chunk_visible={bool(cfg.chunk_visible)} "
