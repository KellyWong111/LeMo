...
OUTDIR=/data1/jingyixi/wm_runs/state_rollout_alignment_fastpilot_seq
mkdir -p "$OUTDIR"
...
train_one() {
  ...
  local model="lewm_encoder_moda_v14_full_visible_bs32_pred6_gate07_staterollseq_${tag}"
...
eval_one() {
  ...
  local out="staterollseq_${tag}_ep${ep}_seed42_h4_s300_k30_n30.txt"
...
outdir = Path('/data1/jingyixi/wm_runs/state_rollout_alignment_fastpilot_seq')
rows = []
for tag in ['l005', 'l010']:
    subdir = stable / f'pusht_encoder_moda_v14_full_visible_bs32_pred6_gate07_staterollseq_{tag}'
    for ep in [1, 2, 4]:
        eval_file = subdir / f'staterollseq_{tag}_ep{ep}_seed42_h4_s300_k30_n30.txt'
        success = None
        if eval_file.exists():
