...
OUTDIR=/data1/jingyixi/wm_runs/cem_success_cost_multi_eval
mkdir -p "$OUTDIR"
...
run_eval() {
  ...
  local tag=$2
  local ep=$3
  local policy_dir="pusht_encoder_moda_v14_full_visible_bs32_pred6_gate07_cemsuccess_multi_${tag}"
  local model="lewm_encoder_moda_v14_full_visible_bs32_pred6_gate07_cemsuccess_multi_${tag}_epoch_${ep}"
...
(
  for ep in 50 100; do run_eval 2 multi_pred_lr1e5_a1e4 "$ep"; done
) > "$OUTDIR/gpu2_pred.log" 2>&1 &

(
  for ep in 50 100; do run_eval 3 multi_predact_lr5e6_a1e4 "$ep"; done
...
outdir = Path("/data1/jingyixi/wm_runs/cem_success_cost_multi_eval")
rows = []
for d in stable.glob("pusht_encoder_moda_v14_full_visible_bs32_pred6_gate07_cemsuccess_multi_*"):
    tag = d.name.split("gate07_cemsuccess_multi_", 1)[-1]
