...
class ExpertClipCEMSolver:
    """CEM with expert-support clipping in normalized action space.

    Eval standardizes actions with a StandardScaler fitted on the expert dataset.
    Therefore the expert action prior is approximately N(0, I) per action
    dimension in the solver/model space. Unlike action shrink, this does not
    reduce all actions. It only clips extreme candidate values outside a chosen
    expert-support band:

        a <- clip(a, -action_clip_std, action_clip_std)

    This is a cheap diagnostic for whether CEM is exploiting learned-model cost
    through tails of the normalized action distribution.
    """
    ...
    def __init__(
        ...
        self.topk = topk
        self.device = device
        self.action_clip_std = float(action_clip_std)
    ...
    def configure(self, *, action_space: gym.Space, n_envs: int, config: Any) -> None:
        ...
        if not isinstance(action_space, Box):
            logging.warning(
                ...
                "ExpertClipCEMSolver may not work as expected."
            )
    ...
    def init_action_distrib(
        ...
        mean = mean.clamp(-self.action_clip_std, self.action_clip_std)
        return mean, var
    ...
    def solve(self, info_dict: dict, init_action: torch.Tensor | None = None) -> dict:
        ...
        for start_idx in range(0, self.n_envs, self.batch_size):
            ...
            for _ in range(self.n_steps):
                ...
                candidates = candidates.clamp(-self.action_clip_std, self.action_clip_std)
                candidates[:, 0] = batch_mean.clamp(
                    -self.action_clip_std, self.action_clip_std
                )
        ...
        print(
            f"ExpertClip CEM solve time: {time.time() - start_time:.4f} seconds "
            f"(action_clip_std={self.action_clip_std}, var_scale={self.var_scale})"
        )
        return outputs
