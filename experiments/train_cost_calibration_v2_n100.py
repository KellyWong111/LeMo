...
OUT = ROOT / "cost_calibration_v2_n100_20260529"
...
def main():
    ...
    for split, (train_seeds, val_seeds) in SPLITS.items():
        ...
        for i, m in enumerate(train_meta):
            if m["source"] == "stateroll" and m["stateroll_only_fixable"] and y[np.where(train_mask)[0][i]]:
                weights[i] *= 3.0
        models = []
        bce_model = fit_logistic(X[train_mask], y[train_mask], weights)
        models.append(("bce", predict(bce_model, X), {"model_info": "weighted BCE linear classifier"}))
        pairwise_model = fit_pairwise_ranker(X, y, meta, train_seeds)
        models.append(("pairwise", linear_score(pairwise_model, X), pairwise_model))
        listwise_model = fit_listwise_ranker(X, y, meta, train_seeds)
        models.append(("listwise", linear_score(listwise_model, X), listwise_model))
        combined_model = fit_combined_ranker(X, y, meta, train_seeds)
        models.append(("combined", linear_score(combined_model, X), combined_model))

        for method, cal_scores, model_info in models:
            ...
            rec = {
                "split": split,
                "method": method,
                "model_info": {k: v for k, v in model_info.items() if k not in {"w", "mean", "std"}},
            ...
            for row in val_metrics:
                metric_rows.append({"split": split, "method": method, "stage": "val", **row})
            deployment_rows.append({k: v for k, v in final.items() if k != "selected_rows"})
            all_threshold_rows.extend([{"split": split, "method": method, **r} for r in thr_rows])

    summary = {"records": records}
    (OUT / "cost_calibration_v2_n100.json").write_text(json.dumps(summary, indent=2))
    write_csv(OUT / "cost_calibration_v2_n100.csv", metric_rows)
    write_csv(OUT / "cost_calibration_v2_deployment_n100.csv", deployment_rows)
    write_csv(OUT / "cost_calibration_v2_threshold_grid_n100.csv", all_threshold_rows)

    lines = ["# PAC-MoDA Cost Calibration v2 n100", ""]
    lines.append("Frozen candidate pools. No encoder/predictor/candidate-pool changes. This compares BCE, pairwise, listwise, and combined planning-aware linear calibration heads.")
    lines.append("Deployment is restricted to the previously fixed precision gates. Thresholds are selected on train seeds only; validation labels are not used for threshold or gate choice.")
    lines.append("")
    lines.append("## Main Table: Raw Stateroll Cost vs Calibrated Stateroll Score")
    lines.append("")
    lines.append("|split|method|score|candidate AUC|first-success rank mean|first-success rank median|top1|top3|top5|top10|top30|near-miss|")
    lines.append("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for rec in records:
        split = rec["split"]
        method = rec["method"]
        for row in rec["val_metrics"]:
            if row["pool"] != "stateroll":
                continue
            lines.append(
                f"|{split}|{method}|raw stateroll cost|{row['candidate_auc_raw']:.3f}|"
            ...
            lines.append(
                f"|{split}|{method}|calibrated stateroll score|{row['candidate_auc_calibrated']:.3f}|"
    ...
    lines.append("")
    lines.append("|split|method|pool|raw AUC|cal AUC|raw first rank|cal first rank|raw top1|cal top1|raw top10|cal top10|raw near-miss|cal near-miss|")
    lines.append("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for rec in records:
        split = rec["split"]
        method = rec["method"]
        for row in rec["val_metrics"]:
            lines.append(
                f"|{split}|{method}|{row['pool']}|{row['candidate_auc_raw']:.3f}|{row['candidate_auc_calibrated']:.3f}|"
    ...
    lines.append("")
    lines.append("|split|method|gate|bsl top1|calibrated selector top1|gate selected|gate st-only|gate bsl FP|threshold|fixed|harmed|switches|st-only recovered|")
    lines.append("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    totals = {}
    for rec in records:
        f = rec["final"]
        t = totals.setdefault(rec["method"], {"fixed": 0, "harmed": 0, "switches": 0, "st": 0})
        t["fixed"] += f["fixed"]
        t["harmed"] += f["harmed"]
        t["switches"] += f["switches"]
        t["st"] += f["stateroll_only_recovered"]
        lines.append(
            f"|{f['split']}|{rec['method']}|{f['gate_model']} {f['gate_rule']}|{f['bsl_top1']:.1f}|{f['calibrated_selector_top1']:.1f}|"
            f"{f['gate_selected']}|{f['gate_st_only']}|{f['gate_bsl_fp']}|{f['threshold']:.4f}|"
            f"{f['fixed']}|{f['harmed']}|{f['switches']}|{f['stateroll_only_recovered']}|"
        )
    lines.append("")
    lines.append("## OOF Deployment Totals")
    lines.append("")
    lines.append("|method|fixed|harmed|switches|stateroll-only recovered|")
    lines.append("|---|---:|---:|---:|---:|")
    for method, t in totals.items():
        lines.append(f"|{method}|{t['fixed']}|{t['harmed']}|{t['switches']}|{t['st']}|")
    (OUT / "cost_calibration_v2_n100.md").write_text("\n".join(lines) + "\n")
    print((OUT / "cost_calibration_v2_n100.md").read_text())
