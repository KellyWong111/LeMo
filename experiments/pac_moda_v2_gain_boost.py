...
OUT = ROOT / "pac_moda_v2_gain_boost_20260529"
...
def fixed_gate_selected(split: str):
    ...
    raise RuntimeError(f"missing gate {split}")


def by_episode(meta, y, scores, seeds, mode="top1"):
    out = {}
    for i, m in enumerate(meta):
...
def evaluate(split, meta, y, scores, val_seeds, threshold, agreement_mask=None):
    ...
    return {"split": split, "threshold": float(threshold), "fixed": fixed, "harmed": harmed, "net": fixed - harmed, "switches": switches, "stateroll_only_recovered": st, "bsl_top1": bsl_succ / total * 100.0, "selector_top1": (bsl_succ + fixed - harmed) / total * 100.0, "gate_selected": len(gate_set), "gate_rule": gate_row["rule"], "selected_rows": rows}


def train_thresholds(split, meta, y, scores, train_seeds, harmed_budget):
    # Train-only threshold selection, but still restricted to the split's fixed gate rows.
    gate_set, _ = fixed_gate_selected(split)
    ep = by_episode(meta, y, scores, train_seeds)
    ...
    rows = []
    for thr in thrs:
        r = evaluate(split, meta, y, scores, train_seeds, thr)
        rows.append(r)
...
def main():
    ...
    for split, (train_seeds, val_seeds) in SPLITS.items():
        ...
        for a, b, c in weight_grid:
            ...
            for hb in [0, 1, 2, 3]:
                thr, _ = train_thresholds(split, meta, y, scores, train_seeds, hb)
                r = evaluate(split, meta, y, scores, val_seeds, thr)
        ...
        for hb in [0, 1, 2, 3]:
            thr, train_curve = train_thresholds(split, meta, y, rank, train_seeds, hb)
            r = evaluate(split, meta, y, rank, val_seeds, thr)
        ...
        for hb in [0, 1, 2, 3]:
            thr, _ = train_thresholds(split, meta, y, rank, train_seeds, hb)
            r = evaluate(split, meta, y, rank, val_seeds, thr, agreement_mask=agree)
    ...
    for hb in [0, 1, 2, 3]:
        ...
        combos = sorted({(r["w_bce"], r["w_rank"], r["w_raw"]) for r in ensemble_rows if r["harmed_budget"] == hb})
        best = None
        for combo in combos:
            rows = [r for r in ensemble_rows if r["harmed_budget"] == hb and (r["w_bce"], r["w_rank"], r["w_raw"]) == combo]
            if len(rows) != 2:
                continue
            agg = {"method": "ensemble_oof", "harmed_budget": hb, "w_bce": combo[0], "w_rank": combo[1], "w_raw": combo[2], "fixed": sum(r["fixed"] for r in rows), "harmed": sum(r["harmed"] for r in rows), "net": sum(r["net"] for r in rows), "switches": sum(r["switches"] for r in rows), "stateroll_only_recovered": sum(r["stateroll_only_recovered"] for r in rows)}
